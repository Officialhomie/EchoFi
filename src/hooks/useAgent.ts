...
import { InvestmentAgent, type InvestmentConfig, type InvestmentResult, type PortfolioBalance } from '@/lib/agent';
...
export interface UseInvestmentAgentReturn {
  ...
  executeStrategy: (strategy: string, amount: string, asset?: string) => Promise<InvestmentResult>;
...
export function useInvestmentAgent(options: UseInvestmentAgentOptions = {}): UseInvestmentAgentReturn {
  ...
  const ensureAgent = useCallback((agent: InvestmentAgent | null): InvestmentAgent => {
    if (!agent || !isInitialized) {
      throw new Error('Investment Agent not initialized. Call initializeAgent() first.');
    }
    return agent;
  }, [isInitialized]);
  ...
  const executeStrategy = useCallback(async (
    ...
    try {
      ...
      return await readyAgent.executeInvestmentStrategy(strategy, amount, asset);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Strategy execution failed';
      setError(errorMessage);
      throw err;
    }
  }, [agent, ensureAgent]);
  ...
  const rebalance = useCallback(async (targets: Record<string, number>): Promise<InvestmentResult> => {
    try {
      const readyAgent = ensureAgent(agent);
      return await readyAgent.rebalancePortfolio(targets);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Rebalancing failed';
      setError(errorMessage);
      throw err;
    }
  }, [agent, ensureAgent]);
  ...
  const getRecommendations = useCallback(async (
    ...
    try {
      const readyAgent = ensureAgent(agent);
      return await readyAgent.getInvestmentRecommendations(riskTolerance, timeHorizon, portfolioValue);
    } catch (err) {
  ...
  const executeDeFiAction = useCallback(async (
    action: string,
    protocol: string,
    asset: string,
    amount: string,
    ...
    try {
      const readyAgent = ensureAgent(agent);
      return await readyAgent.executeDeFiAction(action, protocol, asset, amount, additionalParams);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'DeFi action failed';
      setError(errorMessage);
      throw err;
    }
  }, [agent, ensureAgent]);
  ...
  const getWalletAddress = useCallback(async (): Promise<string> => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to get wallet address';
      setError(errorMessage);
      throw err;
    }
  }, [agent, ensureAgent]);
